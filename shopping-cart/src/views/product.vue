...
const product = ref(store.produtcs.find(prod => prod.id == props.id))
...
function addToCart(){
  store.addCart({...product.value, quantity: 1})
}
</script>
...
<template>
  ...
  <div class="">
    ...
    <div class="flex items-center gap-16 mt-6">
      ...
      <div v-if="isInCart" class="flex gap-3 items-center justify-between px-2.5 font-semibold w-full text-center bg-cyan-500 text-white rounded-full py-1">
