...
const Todos = ref(['learn vue', 'my project', 'finish project',])
...
const searchTodo = ref(null)
const filterTodos = computed ( () => {
     if(searchTodo.value){
      return Todos.value.filter(task => task.indexOf(searchTodo.value) != -1)
     }else{
      return Todos.value
     }
})
  ...
  <main class="min-h-screen w-full py-20 px-5 flex items-center justify-center bg-cyan-800">
    ...
    <div class="w-full max-w-sm">
      ...
      <div class="flex items-center justify-center text-white space-x-4">
        ...
        <h1 class="font-semibold text-4xl">Todo List</h1>
      </div>
      <!-- Todos title end -->

      <!-- Todos search -->
      <div class="mt-5 relative">
        <input type="text" v-model="searchTodo"
      ...
      <div class="mt-6 space-y-2">
        ...
        <div v-for="(todo, index) in filterTodos" :key="index"
             class="pl-4 pr-2.5 py-1.5 text-white rounded bg-cyan-700/90 relative flex items-center justify-between">
          <p>{{ todo }}</p>
          <!-- Delete list -->
          <button @click="RemoveTodo(index)" class="opacity-50 hover:opacity-100 active:scale-90 transition">
